...
#this creates the priority queue for the frontier
class PriorityQueue:
    def __init__(self):
        self.queue = []
        self._index = 0
...
#heuristic cost + heuristic Astar
def Astar(initial, r, c):
    moves = {
        1: (1, -1),
        2: (1, 0),
        3: (1, 1),
    ...
    initialh = heuristic(initial)
    frontier.push([], initialh)
    score = 50
    ...
    while len(frontier) != 0:
        ...
        if isGoal(check, r, c):
            ...
            return
        valid = possMoves(moves, check, r, c)
        if valid != None:
            ...
            for y in valid:
                appender = current[:]
                appender.append(y)
                copy2 = customcopy(initial)
                double = moveOnState(r, c, appender, moves, copy2)
                cplush = (cost(appender)) + (heuristic(double))
                amLdouble = actManPos(double, r, c)
                if amLdouble != None:
                    frontier.push(appender, cplush)


#calculates the cost of a move list
def cost(moves):
    movelist = moves
    shoots = ['N', 'S', 'E', 'W']
    change = 0
...
#the heuristic to a finshed board state 5* enemycounter
def heuristic(boardstate):
    ogre, demon = enemyCounter(boardstate)
    change = (ogre + demon)*5
    return change

#compares the board state to a winner one
def isGoal(state, r, c):
